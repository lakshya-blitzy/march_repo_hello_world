...
A minimal Node.js HTTP server that returns a static `Hello, World!` plain-text response to every request. `Source: server.js:L17, L45-L49`
...
headers, or body — with the same fixed plain-text response, `Hello, World!`. `Source: server.js:L17, L25, L45-L49`

It is intentionally minimal: there is no framework, no router, no configuration layer, and no
third-party dependency. The complete program lives in a single file, `server.js`. `Source: server.js:L17; repository root inventory`

### Features

- **Zero dependencies** — uses only the Node.js built-in `http` module; there is nothing to install. `Source: server.js:L17`
- **Loopback binding** — listens on `127.0.0.1:3000` (the local machine only). `Source: server.js:L25, L31`
- **Catch-all response** — any method on any path returns the identical response. `Source: server.js:L45-L49`
- **Startup log** — prints the running URL to stdout once the listener is ready. `Source: server.js:L57-L59`
...
- A **Node.js runtime** on any currently supported (non-EOL) LTS line — for example **v22**
  ...
  module this server uses, so no additional runtime components are required. `Source: server.js:L17`
...
> `package.json` and no lockfile. `Source: server.js:L17; repository root inventory`
...
exactly (`Source: server.js:L57-L59`):

```text
Server running at http://127.0.0.1:3000/
```

The process stays in the foreground and keeps listening until you stop it (press `Ctrl+C`). `Source: server.js:L57-L59`
...
`Content-Length: 14`). `Source: server.js:L45-L49`

### Request lifecycle

The diagram below shows that any client request — regardless of method or path — flows through the
HTTP listener to the request handler and yields the same static response. `Source: server.js:L45-L49`
...
machine**. `Source: server.js:L25` Requests from other hosts will not reach it as-is. To make the
...
> and `port` remains `3000`. `Source: server.js:L25, L31`
...
block below. `Source: server.js:L17-L59`
...
- **`[L12-L14]` — Start listening and log (the startup callback).** `server.listen(port, hostname, callback)`
  ...
  `Source: server.js:L57-L59`

### Component flow

The diagram below shows the three logical components inside `server.js` — the HTTP listener, the
request handler, and the startup logger — and the one-time startup log. `Source: server.js:L17-L59`
...
no configuration file — `hostname` and `port` are hardcoded constants. `Source: server.js:L25, L31`
